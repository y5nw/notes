...
\section{Funktionen}
\begin{definition}
  Eine Relation $R = (A, B, G)$ heißt eine Funktion (oder Abbildung)\index{Funktion}, wenn $\forall a \in A\: \exists! b = B: (a, b) \in G$.

  Man setzt $f(a) := b$.

  $A$ heißt Definitionsbereich\index{Funktion!Definitionsmenge}, $B$ heißt Wertebereich (Zielmenge)\index{Funktion!Zielmenge}.
\end{definition}
\begin{example}
  \begin{align*}
...
\begin{definition}
  ...
  \begin{itemize}
  \item $M \subseteq A: f(M) := \left\{ b \in B \middle| \exists x \in M: b = f(x) \right\}$ das Bild\index{Funktion!Bild}\index[sym]{f(M)@$f(M)$} von $M$.
  \item $N \subseteq B: f^{-1}(N) := \left\{ a \in A \middle| f(a) \in N \right\}$ das Urbild\index{Funktion!Urbild}\index[sym]{f-1(N)@$f^{-1}(N)$} von $N$
  \end{itemize}

  Ist $f: A \to B$ eine Funktion, dann gibt es ein $f^{-1}: \mathscr{P}(B) \to \mathscr{P}(A)$\index[sym]{f-1@$f^{-1}$}

  Ist $f: A \to B$ und $M \subseteq A$, dann ist $f|_M: M \to B$ eine Einschränkung\index{Funktion!Restriktion} von $f$ auf $M$.
\end{definition}

\begin{definition}
  Es sei $f: A \to B$, dann ist $f$
  \begin{itemize}
  \item injektiv\index{Funktion!Injektiv}, wenn $\forall a_1, a_2 \in A: f(a_1) = f(a_2) \implies a_1 = a_2$
  \item surjektiv\index{Funktion!Surjektiv}, wenn $f(A) = B$
  \item bijektiv\index{Funktion!Bijektiv}, wenn $f$ injektiv und surjektiv ist.
  \end{itemize}
...
\begin{definition}
  ...
  $T \subseteq A$ heißt geordnet\index{Menge!Geordnete Menge@(Teilweise) Geordnete Menge}, wenn es eine Kette gibt mit $a_1, a_2 \in T \implies a_1 \prec a_2 \vee a_2 \prec a_1$.
\end{definition}
\begin{example}
